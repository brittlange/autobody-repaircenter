...
#Marketing campaign analysis starts here

#Average Repair Shop Financial Metrics
average_repair_order <- 2400  #Average repair is about 2,400
...
create_col <- function(spend) {
  ...
  ROAS <- revenue/spend
  market_share_lif <- conversion / marketing_campaign_df$Predicted_NYC_Collisions[12]
  grs_prf <- revenue * gross_profit_margin
  net_prft <- revenue * net_profit_margin
  ret_on_inv <- net_prft/spend
  
  round(c(clicks, conversion, revenue, ROAS, market_share_lif, grs_prf, net_prft, ret_on_inv),4)
}


#create and display the dataframe for the three levels of spend
